...
def add_ip(data: StrModel):
    ...
    if not any(entry["src"] == src and entry["dst"] == dst for entry in ip_data):
        ip_data.append({"src": src, "dst": dst, "marker_color": "fluency/16/full-stop.png"})
        #update_map()
...
def add_url(data: StrModel):
    ...
    if not any(entry["src"] == src and entry["dst"] == dst for entry in ip_data):
        ip_data.append({"src": src, "dst": dst, "marker_color":"color/16/sphere.png"})
        #update_map()
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        await asyncio.sleep(5)  # Every 5 seconds, push update
        markers = get_marker_data()
        await websocket.send_json(markers)
